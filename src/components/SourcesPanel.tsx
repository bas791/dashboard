...
export function SourcesPanel({ rows }: { rows: SourceRow[] }) {
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-md shadow-slate-900/5">
      <h2 className="mb-2 shrink-0 text-xl font-bold text-slate-900">Lead Sources Today</h2>
      {top.length === 0 ? (
        <p className="py-1 text-lg text-slate-400">No enquiries yet today.</p>
      ) : (
            ...
            <li key={row.source}>
              <div className="mb-0.5 flex items-baseline justify-between gap-3 text-base leading-tight">
                ...
                <span className="shrink-0 tabular-nums text-slate-900">
                  ...
                  <span className="text-slate-500"> lead{row.total === 1 ? "" : "s"}</span>
                  {row.won > 0 && (
                    <span className="text-emerald-600"> · {row.won} closed</span>
                  )}
                </span>
              </div>
